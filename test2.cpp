...
int restore (DIR curr_dir, cell curr_cell, cube &resulting_cube, int counter)
{
	...
	if (curr_cell==starting_cell) {
		...
		cout<<"kzl\n";
		cout<<counter<<" "<<curr_cell.x<<" "<<curr_cell.y<<" "<<resulting_cube.bottom<<endl;
		...
		for (i=0; i<4; i++) {
			...
			{
				curr_sum = restore((DIR)i,prev_cell,curr_cube,counter+1) + curr_cube.bottom;
				if (curr_sum < min_sum)
				{
					min_sum = curr_sum;
...
int main()
{
	...
	cout << restore(UP,ending_cell,resulting_cube,0) << endl;
